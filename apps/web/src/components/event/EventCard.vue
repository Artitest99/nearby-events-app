<template>
  <article
    class="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm transition duration-200 hover:-translate-y-[1px] hover:shadow-md">
    <div class="flex flex-col lg:flex-row">
      <div v-if="event.imageUrl" class="h-56 w-full shrink-0 bg-slate-100 lg:h-auto lg:w-[300px] xl:w-[340px]">
      ...
      <div class="min-w-0 flex-1 p-5">
        <div class="space-y-4">
          <div class="flex flex-wrap items-center gap-2 text-xs font-medium text-slate-500">
            ...
            <span v-if="event.distanceKm != null"
              class="inline-flex items-center gap-1 rounded-full bg-slate-100 px-3 py-1 text-slate-700">
              <Route class="h-3.5 w-3.5" />
              {{ event.distanceKm }} km
            </span>

            <span v-if="event.score != null" class="rounded-full bg-sky-50 px-3 py-1 text-sky-700">
              {{ scorePercent }}%
            </span>

            <span v-if="hasGroupedOccurrences" class="rounded-full bg-amber-50 px-3 py-1 text-amber-700">
          ...
          <div class="min-w-0">
            ...
            <p v-if="event.venueName" class="mt-2 flex min-w-0 items-start gap-2 text-sm text-slate-600">
              <MapPin class="mt-0.5 h-4 w-4 shrink-0 text-slate-400" />
              <span class="min-w-0 truncate" :title="`${event.venueName}${event.city ? ' · ' + event.city : ''}`">
                {{ event.venueName }}
          ...
          <div class="grid gap-3 sm:flex sm:flex-wrap">
            <RouterLink :to="`/event/${encodeURIComponent(event.providerEventId)}`"
          ...
          <div v-if="hasGroupedOccurrences" class="rounded-2xl border border-slate-200 bg-slate-50 p-4">
            <button type="button" class="flex w-full items-center justify-between gap-4 text-left"
              ...
              <ChevronDown class="h-4 w-4 text-slate-500 transition duration-200"
                :class="showOccurrences ? 'rotate-180' : ''" />
            </button>

            <transition name="expand-fade">
              <div v-if="showOccurrences" class="mt-4 space-y-3">
                <div v-for="occurrence in groupedOccurrences" :key="occurrence.id"
                  ...
                  <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                    ...
                    <div class="grid gap-2 sm:flex sm:flex-wrap">
                      <RouterLink :to="`/event/${encodeURIComponent(occurrence.providerEventId)}`"
                      ...
                      <a v-if="occurrence.externalUrl" :href="occurrence.externalUrl" target="_blank" rel="noreferrer"
