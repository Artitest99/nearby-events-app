    ...
    <div class="flex flex-col gap-2">
      <div class="flex items-center gap-1.5">
        <div class="inline-flex rounded-xl border border-slate-300 bg-slate-50 p-1">
          <button type="button"
            class="inline-flex items-center gap-1.5 rounded-lg px-2.5 py-1.5 text-sm font-medium transition" :class="viewMode === 'list'
          ...
          <button type="button"
            class="inline-flex items-center gap-1.5 rounded-lg px-2.5 py-1.5 text-sm font-medium transition" :class="viewMode === 'map'
              ? 'bg-white text-slate-900 shadow-sm'
              : 'text-slate-600'
...
class="w-full rounded-xl border border-slate-300 bg-white py-1.5 pl-9 pr-3 text-sm text-slate-700 outline-none transition focus:border-sky-500">            @change="emitSortChange">
      ...
      <div class="flex gap-2 overflow-x-auto pb-0.5">
        <button type="button" class="shrink-0 rounded-full px-3 py-1 text-xs font-medium transition" :class="!activeTagFilter
        ...
        <button v-for="item in tagSummaries" :key="item.tag" type="button"
          class="shrink-0 rounded-full px-3 py-1 text-xs font-medium transition" :class="activeTagFilter === item.tag
            ? 'bg-sky-600 text-white'
...
<script lang="ts">
import { ArrowUpDown, List, Map } from 'lucide-vue-next'
